...
internal class MovieGatewayTest : BaseTest() {
  ...
  private val movieV4Service: MovieV4Service = mockk(relaxed = true) {
    coEvery { watchList(aAccountId, 1) } returns aNetworkResponseSuccessFirstPage
    coEvery { watchList(aAccountId, 2) } returns aNetworkResponseSuccessSecondPage
    coEvery { watchList(aAccountId, 3) } returns aNetworkResponseSuccessThirdPage
  }
  ...
  @Test
  fun `Given a movie results split in pages, When watchList is called with a valid accountId, Then return all movies`() = testDispatcher.runBlockingTest {
    val resultData = gateway.watchList(aAccountId)

    coVerify {
      movieV4Service.watchList(aAccountId, 1)
      movieV4Service.watchList(aAccountId, 2)
      movieV4Service.watchList(aAccountId, 3)
    }
    assertEquals(
      ResultData.Success(
        listOf(
