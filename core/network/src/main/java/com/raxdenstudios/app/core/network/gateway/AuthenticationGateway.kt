...
class AuthenticationGateway @Inject constructor(
    ...
    suspend fun requestToken(): ResultData<RequestTokenResponseDto, NetworkError<ErrorDto>> =
        authenticationV4Service.requestToken()
            .toResultData("Error occurred during requesting token")

    suspend fun requestAccessToken(token: String): ResultData<AccessTokenResponseDto, NetworkError<ErrorDto>> =
        authenticationV4Service.requestAccessToken(AccessTokenRequestDto(token))
            .toResultData("Error occurred during requesting token")

    suspend fun requestSessionId(accessToken: String): ResultData<SessionResponseDto, NetworkError<ErrorDto>> =
